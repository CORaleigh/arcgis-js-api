...
class LayerList extends declared(Widget) {
  ...
  render(): VNode {
    ...
    const resetButton =
        <div
          key={`reset-button`}
          bind={this}
          onclick={this._resetLayerVisibility}
          class={this.classes(CSS.actionsMenuItem)}
          //tabindex="0"
          role="button"
        ...
        >
          <span aria-hidden="true" class={CSS.iconToggleVisible} />
        </div>;  
    const filterSection = (
        <div class={this.classes(CSS.filterSection, CSS.esriWidget)} key={`layerlist-filter`}>
          <input
            type="text"
    ...
    const content =
      ...
      items.length === 0 ? (
        <div>{filterSection}<div class={CSS.noItems}>{i18n.noItemsToDisplay}</div></div>
      ) : (
  ...
  private _resetLayerVisibility = function () {
    this.operationalItems.forEach((item) => {
  ...
  private _updateInputValue = function (e) {
    var target = e.target;
