...
interface Sketch extends Widget {
  ...
  on(type: "buffered", listener: (event: CreateEvent) => void): IHandle;
}
...
class Sketch extends declared(Widget) {
  ...
  @property()
  bufferGraphic: __esri.Graphic = null;
  @property()
  propertyLayer: __esri.FeatureLayer = null;
  @property()
  ...
  render(): VNode {
    ...
    return (
      <div aria-label={i18n.widgetLabel} class={classes}>
        ...
        <div class={this.classes(CSS.panel, CSS.infoPanel)}>{this.renderBufferSection()}</div>
  ...
  protected renderMultiPointButton(): VNode {
    const title = "Draw multiple points";
    const classes = [CSS.button, CSS.pointIcon];
    ...
    return (
      <button
        ...
        bind={this}
        class={this.classes(classes)}
        onclick={this._activateCreateMultiPoint}
        title={title}
      />
    );
  }
...
protected renderBufferSection(): VNode {
  const bufferInput =       <input
  type="number"
   data-input-name="distance"
   placeholder="Buffer distance (feet)"
   class={this.classes(CSS.inputText, CSS.input)}
   value={this.distance}
   oninput={this._updateInputValue}
  bind={this}
/>;
const bufferButton =         <button
aria-label= "Buffer Property"
role="button"
class={this.classes(CSS.widgetButton, CSS.bufferButton)}
// tabIndex={0}
onclick={this._bufferProperty}
bind={this}
>
Buffer Property
</button>;
    ...
    (this.bufferGraphic && parseInt(this.distance) != NaN && parseInt(this.distance) > 0) ? (
      <div key={`buffer-section`}>
      {bufferInput}
      {bufferButton}
      </div>
    ) : (
  ...
  private _bufferProperty() {
    if (this.bufferGraphic && parseInt(this.distance) != NaN && parseInt(this.distance) > 0) {
      let geometry = geometryEngine.buffer(this.bufferGraphic.geometry, parseInt(this.distance), 'feet');
  ...
  private _selectProperties(geometry) {
    if (this.propertyLayer) {
      this.propertyLayer.queryFeatures({geometry: geometry, returnGeometry: true, outFields: ['OBJECTID']}).then(result => {
        let oids = [];
        result.features.forEach(feature => {
          oids.push(feature.attributes.OBJECTID);
          
        });            
        this.propertyLayer.queryRelatedFeatures({relationshipId: 0, objectIds: oids, outFields: ['*']}).then(result => {
          let data = [];
          oids.forEach(oid => {
            if (result[oid]) {
              result[oid].features.forEach((f:any) => {
  ...
  private _onOperationComplete(event: CreateEvent | UpdateEvent): void {
    ...
    if (event.state === "complete" || event.state === "cancel") {
      ...
      if (parseInt(this.distance) != NaN && parseInt(this.distance) > 0) {
        let geometry = geometryEngine.buffer(event.graphic.geometry, parseInt(this.distance), 'feet');
  ...
  private _activateCreateMultiPoint(): void {
